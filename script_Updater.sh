...
Check_Script_Update () {
  ...
  if [[ $ScriptVersion < $Online_ScriptVersion ]]; then
    ScriptName="$0"
    echo $ScriptName
    echo "\x1B[1;33mScript not up to date ($ScriptVersion)! \x1B[1;32mDownloading newest version ($Online_ScriptVersion)...\x1B[0m\n"
    ...
    echo "\x1B[1;33mYour version of the script ($ScriptVersion) is newer than the server version ($Online_ScriptVersion).\x1B[0m\n"
  else
    echo "\x1B[1;32mScript is up to date.\x1B[0m\n"
  fi
}
###########################################################################
